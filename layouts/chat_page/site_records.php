...
.showUser {
  opacity: 1 !important; /* Показываем элементы при добавлении класса */
}
</style>
    <ul id="k40w" class='list'>
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  userElements.forEach(function(element) {
    element.classList.add('showUser'); // Показываем элементы по умолчанию
  });
});
...
document.getElementById('toggleButton').addEventListener('click', function() {
  var userElements = document.querySelectorAll('.user');
  userElements.forEach(function(element) {
    element.classList.toggle('showUser'); // Переключаем видимость
  });
});
